...
class OrderService
{
     ...
     * @link      http://utf8.hk/
     */
    public function validatorGoodsBak($gid,$by_amount): Goods
    {
        // 获得商品详情
        $goods = $this->goodsService->detail($gid);
        // 商品状态验证
        $this->goodsService->validatorGoodsStatus($goods);
        // 如果有限购
        if ($goods->buy_limit_num > 0 && $by_amount > $goods->buy_limit_num) {
            throw new RuleValidationException(__('dujiaoka.prompt.purchase_limit_exceeded'));
        }
        // 库存不足
        if ($by_amount > $goods->in_stock) {
            throw new RuleValidationException(__('dujiaoka.prompt.inventory_shortage'));
        }
        return $goods;
    }
     ...
     * @link      http://zhangyiqiu.net/
     */
    public function validatorLoopCarmisBak($gid,$by_amount)
    {
        $carmis = Carmis::query()
            ->where('goods_id', $gid)
            ->where('status', Carmis::STATUS_UNSOLD)
